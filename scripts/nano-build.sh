...
mkdir -p /tmp/onnx-trt && cd /tmp/onnx-trt && rm -fr onnx-tensorrt &&\
    ...
    mkdir build && cd build && cmake .. -DProtobuf_DEBUG=ON -DProtobuf_USE_STATIC_LIBS=ON \
				     -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/tmp/onnx-trt/install -DGPU_ARCHS="53" && make -j && make install && \
    mv -f /tmp/onnx-trt/install/include/* /usr/include/aarch64-linux-gnu/include && mv -f /tmp/onnx-trt/install/lib/libnvonnx* /usr/lib/aarch64-linux-gnu/ && ldconfig

cd nemo && pip3 install -e . && cd ../collections/nemo_asr && pip3 install -e .
